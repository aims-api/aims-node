...
const pathPlaylist = `${API_HOST}/${API_VERSION}/playlist/add-track/by-file-url`
const pathCustomTag = `${API_HOST}/${API_VERSION}/custom-tag/add-track/by-file-url`
...
describe('Collections - addTrack - byFileUrl', () => {
  ...
  describe('Custom tag', () => {
    test('success, return data', async () => {
      mockServerBodyData(200)
      const response = await testClient.endpoints.customTag.addTrack.byFileUrl({
        track: 'link',
        collection_key: 'abc',
      })
      expect(response).toEqual({
